...
public class TextUtil {
    ...
    public static String[] colorize(String[] lines) {
        return TextUtil.colorize(Arrays.asList((String[])lines.clone())).toArray(new String[0]);
    }

    public static List<String> colorize(List<String> s) {
        ArrayList<String> colorized = new ArrayList<String>();
        s.forEach(st -> colorized.add(TextUtil.colorize(st)));
        return colorized;
    }

    public static TextComponent msgHoverClick(String msg, String hover, String click, ClickEvent.Action clickAction) {
        TextComponent tc = new TextComponent(TextUtil.colorize(msg));
        tc.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new ComponentBuilder(TextUtil.colorize(hover)).create()));
